...
def sizes(request):
    ...
    if res_type != "json" and incl_cum:
        return response("Sorry buddy. Cumulative sizes not yet supported for non-JSON file types. 😞", status=501)
...
def viewconfs(request):
    ...
    if request.method == "POST":
        ...
        if len(existing_object) > 0:
            return JsonResponse(
                {"error": "Object with uid {} already exists".format(uid)}, status=rfs.HTTP_400_BAD_REQUEST
            )

        serializer = tss.ViewConfSerializer(data={"viewconf": viewconf})

        if not serializer.is_valid():
            return JsonResponse({"error": "Serializer not valid"}, status=rfs.HTTP_400_BAD_REQUEST)
...
def tiles(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return JsonResponse({"error": "Unable to parse request body as JSON."}, status=rfs.HTTP_400_BAD_REQUEST)
...
def tileset_info(request):
    ...
    if "cs" in request.GET:
        ...
        if "ci" in request.GET:
            try:
                ...
                data = tcs.chromsizes_array_to_series(tcs.get_tsv_chromsizes(chromsizes.datafile.path))
            except Exception as ex:
                pass
    ...
    for tileset_uuid in tileset_uuids:
        ...
        if tileset_object.filetype == "hitile" or tileset_object.filetype == "hibed":
            tileset_info = hdft.get_tileset_info(h5py.File(tileset_object.datafile.path, "r"))
            tileset_infos[tileset_uuid] = {
                "min_pos": [int(tileset_info["min_pos"])],
                "max_pos": [int(tileset_info["max_pos"])],
                "max_width": 2 ** math.ceil(math.log(tileset_info["max_pos"] - tileset_info["min_pos"]) / math.log(2)),
            ...
            chromsizes = tgt.get_chromsizes(tileset_object)
            tileset_infos[tileset_uuid] = {"chromsizes": [(c, int(s)) for c, s in chromsizes]}
        elif tileset_object.filetype == "fasta":
            chromsizes = tgt.get_chromsizes(tileset_object)
            tsinfo = hgfa.tileset_info(tileset_object.datafile.path, chromsizes)
            # print('tsinfo:', tsinfo)
            if "chromsizes" in tsinfo:
                tsinfo["chromsizes"] = [(c, int(s)) for c, s in tsinfo["chromsizes"]]
            tileset_infos[tileset_uuid] = tsinfo
            ...
            tileset_infos[tileset_uuid] = hgmu.tileset_info(tileset_object.datafile.path)
            ...
            tileset_infos[tileset_uuid] = cdt.get_tileset_info(tileset_object.datafile.path)
        elif tileset_object.filetype == "bed2ddb":
            tileset_infos[tileset_uuid] = cdt.get_2d_tileset_info(tileset_object.datafile.path)
        elif tileset_object.filetype == "cooler":
            tileset_infos[tileset_uuid] = hgco.tileset_info(tileset_object.datafile.path)
        elif tileset_object.filetype == "time-interval-json":
            tileset_infos[tileset_uuid] = hgti.tileset_info(tileset_object.datafile.path)
        elif tileset_object.filetype == "2dannodb" or tileset_object.filetype == "imtiles":
            tileset_infos[tileset_uuid] = hgim.get_tileset_info(tileset_object.datafile.path)
        elif tileset_object.filetype == "geodb":
            tileset_infos[tileset_uuid] = hggo.tileset_info(tileset_object.datafile.path)
            ...
            # Unknown filetype
            tileset_infos[tileset_uuid] = {"error": "Unknown filetype " + tileset_object.filetype}
...
def link_tile(request):
    ...
    try:
        obj = tm.Tileset.objects.create(
            datafile=diff_path, name=op.basename(body["filepath"]), owner=request.user, **tile_data
        )
    except Exception as e:
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not op.isfile(filepath):
                return JsonResponse(
                    {"error": "Unable to locate tileset media file for deletion: {}".format(filepath)}, status=500
                )
            os.remove(filepath)
        except dh.Http404:
