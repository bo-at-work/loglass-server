...
def add_file(filename, sub_dir="uploads/data"):
    ...
    def test_list_tilesets(self):
        ...
        ret = self.client.get("/api/v1/list_tilesets/?ui={}".format(private_project["uuid"]))
    ...
    def test_create(self):
        ...
        ret = self.client.post(
            "/api/v1/projects/", '{"name": "test project", "owner": "user1" }', content_type="application/json"
        )
        ...
        body = [
            {
                "tilesetUid": "chr21_KL",
                "tileIds": ["0.0", "0.1"],
                "options": {"aggGroups": [[0, 1], [2, 3, 4], [5, 6]], "aggFunc": "sum"},
            }
        ]
...
class ChromosomeSizes(dt.TestCase):
    def test_tileset_info(self):
        ...
        ret = self.client.get("/api/v1/tileset_info/?d=cs-hg19").json()
...
class ViewConfTest(dt.TestCase):
    ...
    def test_viewconfs(self):
        ret = self.client.post(
            "/api/v1/viewconfs/", '{"uid": "123", "viewconf":{"hello": "sir"}}', content_type="application/json"
        )
        contents = json.loads(ret.content.decode("utf-8"))
    ...
    def test_duplicate_uid_errors(self):
        ret1 = self.client.post(
            "/api/v1/viewconfs/", '{"uid": "dupe", "viewconf":{"try": "first"}}', content_type="application/json"
        )
        ...
        if hss.UPLOAD_ENABLED:
            ...
            # really be 4xx.
            ret = self.client.post(
                "/api/v1/viewconfs/", '{"uid": "dupe", "viewconf":{"try": "second"}}', content_type="application/json"
            )
...
class PermissionsTest(dt.TestCase):
    ...
    def test_permissions(self):
        ...
        if hss.UPLOAD_ENABLED:
            ...
            resp = c2.put(
                "/api/v1/tilesets/" + ret["uuid"] + "/", data='{"name":"newname"}', content_type="application/json"
            )
            assert resp.status_code == 403
...
class BigBedTest(dt.TestCase):
    ...
    def test_get_tiles_via_post(self):
        ...
        body = [{"tilesetUid": "bb", "tileIds": ["14.12"]}]
...
class BedDBTest(dt.TestCase):
    ...
    def test_get_tiles(self):
        ...
        returned_text = self.client.get(
            "/api/v1/tiles/?d={tile_id}&d={tile_id1}".format(tile_id=tile_id, tile_id1=tile_id1)
        )
        returned = json.loads(returned_text.content.decode("utf-8"))
...
class TilesetsViewSetTest(dt.TestCase):
    def setUp(self):
        ...
        upload_file = open("data/dixon2012-h1hesc-hindiii-allreps-filtered.1000kb.multires.cool", "rb")
        self.cooler = tm.Tileset.objects.create(
            datafile=dcfu.SimpleUploadedFile(upload_file.name, upload_file.read()), filetype="cooler", owner=self.user1
        )
        ...
        self.hitile = tm.Tileset.objects.create(
            datafile=dcfu.SimpleUploadedFile(upload_file.name, upload_file.read()), filetype="hitile", owner=self.user1
        )
    ...
    def check_tile(self, z, x, y):
        returned = json.loads(
            self.client.get(
                "/api/v1/tiles/?d={uuid}.{z}.{x}.{y}".format(uuid=self.cooler.uuid, x=x, y=y, z=z)
    ...
    def test_post_dataset(self):
        ...
        ret = c.post(
            "/api/v1/tilesets/",
            {"datafile": f, "filetype": "hitile", "private": "True", "coordSystem": "hg19"},
            format="multipart",
        )
        f.close()
    ...
    def test_create_private_tileset(self):
        ...
        c.login(username="user1", password="pass")
        returned = json.loads(
            self.client.get("/api/v1/tileset_info/?d={uuid}".format(uuid=private_obj.uuid)).content.decode("utf-8")
        )
    ...
    def test_get_many_tiles(self):
        ...
        returned = json.loads(
            self.client.get(
                "/api/v1/tiles/?d={uuid}.1.0.0&d={uuid}.1.0.1".format(uuid=self.cooler.uuid)
            ).content.decode("utf-8")
        )
    ...
    def test_get_same_tiles(self):
        ...
        returned = json.loads(
            self.client.get(
                "/api/v1/tiles/?d={uuid}.1.0.0&d={uuid}.1.0.0".format(uuid=self.cooler.uuid)
            ).content.decode("utf-8")
        )
    ...
    def test_get_nonexistent_tile(self):
        ...
        returned = json.loads(
            self.client.get("/api/v1/tiles/?d={uuid}.1.5.5".format(uuid=self.cooler.uuid)).content.decode("utf-8")
        )
        ...
        returned = json.loads(
            self.client.get("/api/v1/tiles/?d={uuid}.20.5.5".format(uuid=self.cooler.uuid)).content.decode("utf-8")
        )

        self.assertTrue("{uuid}.1.5.5".format(uuid=self.cooler.uuid) not in returned.keys())

    def test_get_hitile_tileset_info(self):
        returned = json.loads(
            self.client.get("/api/v1/tileset_info/?d={uuid}".format(uuid=self.hitile.uuid)).content.decode("utf-8")
        )
    ...
    def test_get_cooler_tileset_info(self):
        returned = json.loads(
            self.client.get("/api/v1/tileset_info/?d={uuid}".format(uuid=self.cooler.uuid)).content.decode("utf-8")
        )
    ...
    def test_get_hitile_tile(self):
        returned = json.loads(
            self.client.get("/api/v1/tiles/?d={uuid}.0.0".format(uuid=self.hitile.uuid)).content.decode("utf-8")
        )
