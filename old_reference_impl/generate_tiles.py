...
def generate_1d_tiles(filename, tile_ids, get_data_function, tileset_options):
    ...
    for tile_id in tile_ids:
        ...
        if not has_nan and max_dense > min_f16 and max_dense < max_f16 and min_dense > min_f16 and min_dense < max_f16:
            tile_value = {
...
def get_chromsizes(tileset):
    ...
    try:
        chrom_info_tileset = tm.Tileset.objects.get(coordSystem=tileset.coordSystem, datatype="chromsizes")
    except:
        return None
...
def generate_hitile_tiles(tileset, tile_ids):
    ...
    for tile_id in tile_ids:
        ...
        dense = hdft.get_data(h5py.File(tileset.datafile.path), tile_position[0], tile_position[1])
        ...
        if not has_nan and max_dense > min_f16 and max_dense < max_f16 and min_dense > min_f16 and min_dense < max_f16:
            tile_value = {"dense": base64.b64encode(dense.astype("float16")).decode("utf-8"), "dtype": "float16"}
        else:
            tile_value = {"dense": base64.b64encode(dense.astype("float32")).decode("utf-8"), "dtype": "float32"}
...
def generate_bed2ddb_tiles(tileset, tile_ids, retriever=cdt.get_2d_tiles):
    ...
    partitioned_tile_ids = list(it.chain(*[partition_by_adjacent_tiles(t) for t in tile_ids_by_zoom]))
    ...
    for tile_group in partitioned_tile_ids:
        ...
        tile_data_by_position = retriever(cached_datapath, zoom_level, minx, miny, maxx - minx + 1, maxy - miny + 1)

        tiles = [
            (".".join(map(str, [tileset_id] + [zoom_level] + list(position))), tile_data)
...
def generate_hibed_tiles(tileset, tile_ids):
    ...
    for tile_id in tile_ids:
        tile_id_parts = tile_id.split(".")
        tile_position = list(map(int, tile_id_parts[1:3]))
        dense = hdft.get_discrete_data(h5py.File(tileset.datafile.path), tile_position[0], tile_position[1])
...
def generate_tiles(tileset_tile_ids):
    ...
    if tileset.filetype == "hitile":
        ...
        chromsizes = get_chromsizes(tileset)
        return hgfa.tiles(
            tileset.datafile.path, tile_ids, chromsizes=chromsizes, max_tile_width=hss.MAX_FASTA_TILE_WIDTH
        )
        ...
        return generate_1d_tiles(tileset.datafile.path, tile_ids, ctmu.get_single_tile, tileset_options)
